...
class TokenDevIT extends IntegrationTestBase {
    ...
    void especificacaoOpenApiDescreveSoAApi() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
                ...
                .andExpect(jsonPath("$.paths['/api/v1/privacidade'].put").exists())
                .andExpect(jsonPath("$.paths['/api/v1/posts'].post.requestBody.content['multipart/form-data']").exists())
                .andExpect(jsonPath("$.paths['/api/v1/posts'].post.requestBody.content['application/json']").exists())
                .andExpect(jsonPath("$.paths['/api/v1/perfil/foto'].put").exists())
                .andExpect(jsonPath("$.paths['/arquivos/{pasta}/{nome}']").doesNotExist())
                .andExpect(jsonPath("$.paths['/api/v1/usuarios/{username}/bloqueio'].put").exists())
